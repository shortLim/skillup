	...
	<div>
		<table class="table table-borderd table-hover">
			...
			<tbody>
				...
				<tr>
					<td>1</td>
					<td><a href="boardRead.jsp">sample(comment})</a></td>
					<td>kim</td>
					<td>2021/09/03</td>
				</tr>
				<tr>
				...
				<c:forEach var="board" items="${list }">
					<c:if test="${board.BNo eq 0}">
					<tr>
						<td>${board.PNo}</td>
						<td><a href="${board.PNo}" class="move"> ${board.title }
								<!-- コメント数表示 --> <c:if test="${ board.CCnt > 0 }">
									<b>[${board.CCnt }]</b>
								</c:if>
						</a></td>
						<td>${board.name }</td>
		...
		<div class="row">
			<div class="col-lg-12">
				<form id="searchForm" action="/board/listSell">
					<select name="type">
		...
		<!-- ページナンバーリンク処理 -->
		<form action="/board/listBuy" id="actionForm" method="get">
